...
sigma_list = [2.0 for _ in range(len(mu_list))]
...
avg_rewards = []
q_array = np.zeros(shape=len(mu_list),dtype=np.float32)
N_array = np.zeros(shape=len(mu_list),dtype=np.float32) + 1e-8
q_tot_array = np.zeros(shape=len(mu_list),dtype=np.float32)
c = 2
for time in range(time_limit):
    uncertainty = c * np.sqrt(np.log(time + 1.1)/N_array)
    action = np.argmax(q_array + uncertainty)
...
fig = plt.plot(range(time_limit),avg_rewards,label=f"UCB : {c}")
plt.legend()
